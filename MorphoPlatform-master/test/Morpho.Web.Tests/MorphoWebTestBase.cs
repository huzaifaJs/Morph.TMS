...
using Morpho.Web.Startup;
using AngleSharp.Html.Dom;
using AngleSharp.Html.Parser;
using Microsoft.AspNetCore.Hosting;
using Shouldly;

namespace Morpho.Web.Tests
{
    public abstract class MorphoWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static MorphoWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(MorphoWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<MorphoDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<MorphoDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<MorphoDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<MorphoDbContext, Task<T>> func)
        {
            T result;
